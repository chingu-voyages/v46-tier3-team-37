import { transactionComplete } from '@/types/schemaTypes';
import {
  cva,
  type VariantProps,
  ...
  'flex dark:bg-foregroundPrimary',
  {
    variants: {
      variant: {
        ...
        detailed:
          'bg-[white] flex-col gap-4 shadow-md font-normal text-sm py-4',
      },
      size: {
        default: 'w-full',
      },
...
interface CardFooterVariantProps
  ...
  transactions?: transactionComplete[];
  activeRenters?: Array<{
    email: string | null;
    username: string;
    id: string;
  ...
  return (
    <div className={CardFooterVariants({ variant, size })}>
      ...
      <div className='flex justify-end gap-3'>
        {React.Children.map(btnChildren, (child) => (
          <>{child}</>
        ))}
      </div>
    </div>
